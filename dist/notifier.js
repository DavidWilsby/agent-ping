...
exports.EVENT_PATH = exports.LOCK_PATH = void 0;
...
exports.LOCK_PATH = path.join(CONFIG_DIR, 'extension.lock');
exports.EVENT_PATH = path.join(CONFIG_DIR, 'event.json');
...
function isExtensionActive() {
    try {
        return fs.existsSync(exports.LOCK_PATH);
    }
    catch {
        return false;
    }
 ...
 */
function writeEvent(title, message) {
    try {
        fs.mkdirSync(CONFIG_DIR, { recursive: true });
        fs.writeFileSync(exports.EVENT_PATH, JSON.stringify({ title, message, timestamp: Date.now() }), 'utf-8');
    }
    catch {
        // Best-effort
    }
