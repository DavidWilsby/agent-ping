...
describe('resolveConfig', () => {
  it('returns bundled defaults when no env vars or config file', () => {
    mockReadFileSync.mockImplementation((p: string) => {
      if (String(p).includes('test-plugin-data')) throw new Error('not found');
  ...
  it('config file values override bundled defaults', () => {
    ...
    mockReadFileSync.mockImplementation((p: string) => {
      if (String(p).includes('test-plugin-data')) return JSON.stringify(fileConfig);
      return jest.requireActual<typeof import('fs')>('fs').readFileSync(p, 'utf-8');
    });
    const { resolveConfig } = require('../src/config');
    const config = resolveConfig();
  ...
  it('AGENT_PING_VOLUME env var overrides config', () => {
    ...
    mockReadFileSync.mockImplementation((p: string) => {
      if (String(p).includes('test-plugin-data')) return JSON.stringify({ volume: 30 });
      return jest.requireActual<typeof import('fs')>('fs').readFileSync(p, 'utf-8');
    });
    const { resolveConfig } = require('../src/config');
    const config = resolveConfig();
  ...
  it('clamps volume above 100 to 100', () => {
    mockReadFileSync.mockImplementation((p: string) => {
      if (String(p).includes('test-plugin-data')) return JSON.stringify({ volume: 200 });
      return jest.requireActual<typeof import('fs')>('fs').readFileSync(p, 'utf-8');
    });
    const { resolveConfig } = require('../src/config');
    const config = resolveConfig();
  ...
  it('clamps volume below 0 to 0', () => {
    mockReadFileSync.mockImplementation((p: string) => {
      if (String(p).includes('test-plugin-data')) return JSON.stringify({ volume: -10 });
      return jest.requireActual<typeof import('fs')>('fs').readFileSync(p, 'utf-8');
    });
    const { resolveConfig } = require('../src/config');
    const config = resolveConfig();
  ...
  it('falls back to default for non-numeric volume in config file', () => {
    mockReadFileSync.mockImplementation((p: string) => {
      if (String(p).includes('test-plugin-data')) return JSON.stringify({ volume: 'fifty' });
      return jest.requireActual<typeof import('fs')>('fs').readFileSync(p, 'utf-8');
    });
    const { resolveConfig } = require('../src/config');
    const config = resolveConfig();
  ...
  it('reads config from CLAUDE_PLUGIN_DATA when set', () => {
    process.env.CLAUDE_PLUGIN_DATA = '/tmp/agent-ping-test';
    const configContent = JSON.stringify({ volume: 75 });
    mockReadFileSync.mockImplementation((p: string) => {
      if (String(p) === '/tmp/agent-ping-test/config.json') return configContent;
